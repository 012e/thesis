...
const RichTextEditor: React.FC = () => {
  ...
  const setEditorRootWrapperElement = usePublisher(
    contentEditableWrapperElement$,
  );
  const onRef = (el: HTMLDivElement | null) => {
    setEditorRootWrapperElement(el);
    setContentEditableRef(
      el ? ({ current: el } as React.RefObject<HTMLDivElement>) : null,
    );
  };
      ...
      <RenderRecursiveWrappers wrappers={editorWrappers}>
        <div
          className={classNames(
            styles.rootContentEditableWrapper,
...
const RenderRecursiveWrappers: React.FC<{
  ...
  return (
    <Wrapper>
      <RenderRecursiveWrappers wrappers={wrappers.slice(1)}>
        {children}
      </RenderRecursiveWrappers>
    </Wrapper>
  );
...
const EditorRootElement: React.FC<{
  ...
  React.useEffect(() => {
    ...
    setEditorRootElementRef(
      editorRootElementRef as React.RefObject<HTMLDivElement>,
    );
    setEditorWrapperElementRef(
      wrapperElementRef as React.RefObject<HTMLDivElement>,
    );
    return () => {
...
const Methods: React.FC<{ mdxRef: React.ForwardedRef<MDXEditorMethods> }> = ({
  mdxRef,
}) => {
  const realm = useRealm();

  React.useImperativeHandle(mdxRef, () => {
    return {
      getMarkdown: () => {
      ...
      getSelectionMarkdown: () => {
        ...
        const jsxComponentDescriptors = realm.getValue(
          jsxComponentDescriptors$,
        );
      ...
      <RealmWithPlugins
        plugins={[
          corePlugin({
            ...
            toMarkdownOptions:
              props.toMarkdownOptions ?? DEFAULT_MARKDOWN_OPTIONS,
