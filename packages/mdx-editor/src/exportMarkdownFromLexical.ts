 ...
 * @group Markdown Processing
 */
export type LexicalVisitor = LexicalExportVisitor<
  LexicalNode,
  Mdast.RootContent
>;
...
export function exportLexicalTreeToMdast({
  ...
  function appendToParent<C extends Mdast.RootContent>(
...
function fixWrappingWhitespace(
  ...
  if (node.type === "strong" || node.type === "emphasis") {
    ...
    if (lastChild?.type === "text") {
      const trailingWhitespace = TRAILING_WHITESPACE_REGEXP.exec(
        lastChild.value,
      );
    ...
    if (firstChild?.type === "text") {
      const leadingWhitespace = LEADING_WHITESPACE_REGEXP.exec(
        firstChild.value,
      );
...
export type ToMarkdownExtension = NonNullable<
