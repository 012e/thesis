...
const RemoteMDXEditorRealmContext = createContext(
  RemoteMDXEditorRealmContextValueStub,
);
 ...
 * @group Utils
 */
export const RemoteMDXEditorRealmProvider: FC<PropsWithChildren> = ({
  children,
}) => {
  const [editorMap, setEditorMap] = React.useState<Map<string, Realm>>(
    new Map(),
  );
  ...
  return (
    <RemoteMDXEditorRealmContext.Provider value={contextValue}>
      {children}
    </RemoteMDXEditorRealmContext.Provider>
  );
};

const RemotePluginRegister: FC<{ realm: Realm; editorId: string }> = ({
  realm,
  editorId,
}) => {
  ...
  init: (realm, params) => {
    if (params?.editorId) {
      realm.pub(addComposerChild$, () => (
        <RemotePluginRegister realm={realm} editorId={params.editorId} />
      ));
    }
  },
});
