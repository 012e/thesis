...
export const FrontmatterEditor = ({
  ...
  const yamlConfig = React.useMemo<YamlConfig>(() => {
    ...
    return Object.entries(YamlParser.load(yaml) as Record<string, string>).map(
      ([key, value]) => ({ key, value }),
    );
  }, [yaml]);
  ...
  const onSubmit = React.useCallback(
    ({ yamlConfig }: { yamlConfig: YamlConfig }) => {
      ...
      const yaml = yamlConfig.reduce<Record<string, string>>(
        (acc, { key, value }) => {
          if (key && value) {
            acc[key] = value;
          }
    ...
    <>
      ...
      >
        <Dialog.Portal container={editorRootElementRef?.current}>
          ...
          <Dialog.Content
            className={styles.largeDialogContent}
...
>(({ className, autofocusIfEmpty: _, ...props }, ref) => {
  return (
    <input
      className={classNames(styles.propertyEditorInput, className)}
      {...props}
      ref={ref}
    />
  );
