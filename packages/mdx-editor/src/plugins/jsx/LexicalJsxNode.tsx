...
export class LexicalJsxNode extends DecoratorNode<JSX.Element> {
  ...
  static clone(node: LexicalJsxNode): LexicalJsxNode {
    return new LexicalJsxNode(
      structuredClone(node.__mdastNode),
      structuredClone(node.__importStatement),
      node.__key,
    );
  }
  ...
  static importJSON(serializedNode: SerializedLexicalJsxNode): LexicalJsxNode {
    return $createLexicalJsxNode(
      serializedNode.mdastNode,
      serializedNode.importStatement,
    );
  }
  ...
  createDOM(): HTMLElement {
    return document.createElement(
      this.__mdastNode.type === "mdxJsxTextElement" ? "span" : "div",
    );
  }
...
export function JsxEditorContainer(props: {
  ...
  const descriptor =
    jsxComponentDescriptors.find(
      (descriptor) => descriptor.name === mdastNode.name,
 ...
 */
export function $createLexicalJsxNode(
  mdastNode: MdastJsx,
  importStatement?: ImportStatement,
): LexicalJsxNode {
 ...
 */
export function $isLexicalJsxNode(
  node: LexicalNode | null | undefined,
): node is LexicalJsxNode {
