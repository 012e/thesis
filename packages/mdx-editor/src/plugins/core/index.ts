...
} from "../../exportMarkdownFromLexical";
import {
  ImportPoint,
  MarkdownParseError,
...
export type AdditionalLexicalNode =
...
export const NESTED_EDITOR_UPDATED_COMMAND = createCommand<undefined>(
...
export const readOnly$ = Cell(false, (r) => {
  r.sub(
    r.pipe(readOnly$, withLatestFrom(rootEditor$)),
    ([readOnly, rootEditor]) => {
...
/** @internal */
export const autoFocus$ = Cell<
  | boolean
  | { defaultSelection?: "rootStart" | "rootEnd"; preventScroll?: boolean }
>(false);
...
/** @internal */
export const markdownErrorSignal$ = Signal<{ error: string; source: string }>(
  (r) => {
    r.link(
      r.pipe(
...
export const applyFormat$ = Signal<TextFormatType>((r) => {
  r.sub(
    r.pipe(applyFormat$, withLatestFrom(activeEditor$)),
    ([format, theEditor]) => {
...
export const currentSelection$ = Cell<RangeSelection | null>(null, (r) => {
  r.sub(
    r.pipe(currentSelection$, withLatestFrom(activeEditor$)),
    ([selection, theEditor]) => {
...
/** @internal */
export const mdastExtensions$ = Cell<
  NonNullable<MarkdownParseOptions["mdastExtensions"]>
>([]);
 ...
 * @group Core
 */
export const exportVisitors$ = Cell<
  NonNullable<LexicalConvertOptions["visitors"]>
>([]);
/** @internal */
export const toMarkdownExtensions$ = Cell<
  NonNullable<LexicalConvertOptions["toMarkdownExtensions"]>
>([]);
/** @internal */
export const toMarkdownOptions$ = Cell<
  NonNullable<LexicalConvertOptions["toMarkdownOptions"]>
>({});
 ...
 * @group Core
 */
export const editorRootElementRef$ =
  Cell<React.RefObject<HTMLDivElement> | null>(null);

/**
 * @group Core
 */
export const editorWrapperElementRef$ =
  Cell<React.RefObject<HTMLDivElement> | null>(null);
 ...
 * @group Core
 */
export const contentEditableRef$ = Cell<React.RefObject<HTMLDivElement> | null>(
  null,
);
...
export const insertMarkdown$ = Signal<string>((r) => {
  r.sub(
    r.pipe(insertMarkdown$, withLatestFrom(activeEditor$, inFocus$)),
    ([markdownToInsert, editor, inFocus]) => {
      editor?.update(() => {
...
function rebind() {
  return scan(
    (
      teardowns,
      [subs, activeEditorValue]: [EditorSubscription[], LexicalEditor | null],
    ) => {
      teardowns.forEach((teardown) => {
        if (!teardown) {
          throw new Error(
            "You have a subscription that does not return a teardown",
          );
        }
        teardown();
      });
...
/** @internal */
export const activeEditorSubscriptions$ = Cell<EditorSubscription[]>(
  [],
  (r) => {
 ...
 * @group Core
 */
export const editorInFocus$ = Cell<EditorInFocus | null>(
  null,
  noop,
  (prev, next) => {
 ...
 * @group Core
 */
export const iconComponentFor$ = Cell<(name: IconKey) => React.ReactNode>(
  (name: IconKey) => {
...
/** @internal */
export function Appender<T>(
  cell$: NodeRef<T[]>,
  init?: (r: Realm, sig$: NodeRef<T | T[]>) => void,
) {
 ...
 * @group Core
 */
export const createRootEditorSubscription$ = Appender(
  ...
  (r, sig$) => {
    ...
    r.pub(sig$, [
      ...
      // Export handler
      (rootEditor) => {
        return rootEditor.registerUpdateListener(
 ...
 * @group Core
 */
export const createActiveEditorSubscription$ = Appender(
      ...
      (editor) => {
        return editor.registerCommand(
          ...
          (payload) => {
            ...
            if (theRootEditor) {
              const movingOutside = !theRootEditor
                .getRootElement()
                ?.contains(payload.relatedTarget as Node);
              if (movingOutside) {
                r.pubIn({
                  [inFocus$]: false,
                  [onBlur$]: payload,
...
function tryImportingMarkdown(
  ...
  try {
    ...
    if (
      e instanceof MarkdownParseError ||
      e instanceof UnrecognizedMarkdownConstructError
    ) {
      r.pubIn({
        [markdown$]: markdownValue,
        [markdownProcessingError$]: {
...
/** @internal */
export const editorWrappers$ = Cell<
...
export const convertSelectionToNode$ = Signal<() => ElementNode>((r) => {
  r.sub(
    r.pipe(convertSelectionToNode$, withLatestFrom(activeEditor$)),
    ([factory, editor]) => {
      editor?.update(() => {
        const selection = $getSelection();
        if ($isRangeSelection(selection)) {
 ...
 * @group Core
 */
export const insertDecoratorNode$ = Signal<() => DecoratorNode<unknown>>(
  (r) => {
    r.sub(
      r.pipe(insertDecoratorNode$, withLatestFrom(activeEditor$)),
      ([nodeFactory, theEditor]) => {
        theEditor?.focus(
          () => {
...
export const corePlugin = realmPlugin<{
  ...
  postInit(r, params) {
    const newEditor = createEditor({
      ...
      nodes: [
        ...r.getValue(usedLexicalNodes$),
        ...(params?.additionalLexicalNodes ?? []),
      ],
      onError: (error) => {
        throw error;
      },
