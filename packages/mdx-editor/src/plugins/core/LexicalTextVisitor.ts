...
export const LexicalTextVisitor: LexicalExportVisitor<
  TextNode,
  Mdast.Text | Mdast.Html | MdxJsxTextElement
> = {
  shouldJoin: (prevNode, currentNode) => {
    ...
    if (
      ...
      JOINABLE_TAGS.includes(
        (currentNode as unknown as MdxJsxTextElement).name!,
      )
    ) {
      const currentMdxNode: MdxJsxTextElement =
        currentNode as unknown as MdxJsxTextElement;
      return (
        prevNode.name === currentMdxNode.name &&
        JSON.stringify(prevNode.attributes) ===
          JSON.stringify(currentMdxNode.attributes)
      );
    }
    return false;
  },
