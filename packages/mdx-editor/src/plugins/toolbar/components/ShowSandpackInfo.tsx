...
export const ShowSandpackInfo = () => {
  const [editorInFocus, theEditor, iconComponentFor, sandpackConfig] =
    useCellValues(
      editorInFocus$,
      activeEditor$,
      iconComponentFor$,
  ...
  const sandpackNode = $isCodeBlockNode(editorInFocus!.rootNode)
  ...
  const preset = sandpackConfig.presets.find(
    (preset) => preset.meta === sandpackNode.getMeta(),
  )!;
