...
function decorateWithRef<P extends { className?: string | undefined }>(
  ...
  decoratedProps: Partial<React.PropsWithoutRef<P>> & {
...
function addTooltipToChildren<
  C extends React.ComponentType<{ children: React.ReactNode }>,
>(Component: C) {
  return ({
    title,
    children,
    ...props
  }: React.ComponentProps<C> & { title: string }) => {
    return (
...
export const Root: React.FC<{
  ...
  className?: string;
}> = ({ readOnly, children, className }) => {
  return (
    <RadixToolbar.Root
      className={classNames(
        "mdxeditor-toolbar",
        styles.toolbarRoot,
        { [styles.readOnlyToolbarRoot]: readOnly },
        className,
      )}
      {...(readOnly ? { tabIndex: -1 } : {})}
    >
 ...
 */
export const Button = decorateWithRef(RadixToolbar.Button, {
...
export const ToggleSingleGroupWithItem = React.forwardRef<
  HTMLDivElement,
  Omit<RadixToolbar.ToolbarToggleGroupSingleProps, "type"> & {
    on: boolean;
    title: string;
    disabled?: boolean;
  }
    ...
    <>
      {props.items.length === 1 ? (
        ...
        <RadixSelect.Root value="" onValueChange={props.onChoose}>
          <SelectButtonTrigger title={props.title}>
            {props.children}
          </SelectButtonTrigger>
...
function isConditionalContentsOption(
  option: ConditionalContentsOption | FallbackOption,
): option is ConditionalContentsOption {
...
export const ConditionalContents: React.FC<{
  ...
  const contents = React.useMemo(() => {
    ...
    return option
      ? isConditionalContentsOption(option)
        ? option.contents()
        : option.fallback()
      : null;
