...
export const MdastDirectiveVisitor: (
...
) => MdastImportVisitor<Directives> = (escapeUnknownTextDirectives) => ({
  testNode: (node, { directiveDescriptors }) => {
    if (isMdastDirectivesNode(node)) {
      const descriptor = directiveDescriptors.find((descriptor) =>
        descriptor.testNode(node),
      );
      if (
  ...
  visitNode({ lexicalParent, mdastNode, descriptors }) {
    const isKnown =
      !escapeUnknownTextDirectives ||
      descriptors.directiveDescriptors.some((d) => d.testNode(mdastNode));
    if (isKnown) {
