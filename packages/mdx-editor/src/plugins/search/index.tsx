...
export const editorSearchTextNodeIndex$ =
  Cell<TextNodeIndex>(EmptyTextNodeIndex);
...
export const editorSearchTermDebounced$ = Cell<string>("", (realm) => {
  realm.link(
    ...
    realm.pipe(editorSearchTerm$, realm.transformer(debounceTime(250))),
  );
});
export const editorSearchScrollableContent$ = Cell<HTMLElement | null>(
  null,
  (r) =>
...
export const debouncedIndexer$ = Cell<TextNodeIndex>(
  EmptyTextNodeIndex,
  (realm) => {
    realm.link(
      debouncedIndexer$,
      realm.pipe(
        ...
        realm.transformer(debounceTime(250)),
      ),
    );
  },
);
...
function indexAllTextNodes(root: HTMLElement | null): TextNodeIndex {
  ...
  while ((currentNode = treeWalker.nextNode())) {
    const nodeContent =
      currentNode.textContent?.normalize("NFKD") ??
...
export function* rangeSearchScan(
  searchQuery: string,
  ...
  for (const [start, end] of searchText(allText, searchQuery)) {
    ...
    if (
      ...
      throw new Error(
        "Invalid range: startNode, endNode, startOffset, or endOffset is undefined.",
      );
    }
...
export const searchPlugin = realmPlugin({
  ...
  init(realm) {
    if (typeof CSS.highlights === "undefined") {
      console.warn(
        "CSS.highlights is not supported in this browser. Search functionality will be limited.",
      );
      return;
    }
    ...
    const updateHighlights = (
      ...
      if (ranges.length) {
        ...
        if (!scrollRange)
          throw new Error(
            "error updating highlights, scroll range does not exist",
          );
