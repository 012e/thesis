      ...
      if (state.type === "preview") {
        setTimeout(() => {
          editor?.getEditorState().read(() => {
            ...
            const withAnchorText = $isLinkNode(node)
              ? node.getTextContent().length > 0 && node.getChildrenSize() <= 1
              : false;
        ...
        setTimeout(() => {
          editor.getEditorState().read(() => {
            ...
            const withAnchorText = linkNode
              ? linkNode.getTextContent().length > 0 &&
                linkNode.getChildrenSize() <= 1
              : Boolean(selection?.isCollapsed());

            const text =
              withAnchorText && linkNode ? linkNode.getTextContent() : "";
