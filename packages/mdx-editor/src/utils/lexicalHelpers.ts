...
const WILL_CHANGE_CONTAINING_BLOCK_PROPS = [
  "transform",
  "perspective",
  "filter",
  "backdrop-filter",
...
const CONTAIN_VALUES_CREATING_CONTAINING_BLOCK = [
  "layout",
  "paint",
  "strict",
  "content",
...
function getFixedContainingBlock(
  ...
  while (current) {
    ...
    const hasRelevantWillChange = willChangeProps.some((prop) =>
      WILL_CHANGE_CONTAINING_BLOCK_PROPS.includes(prop),
    );
 ...
 * @group Utils
 */
export function getSelectionAsMarkdown(
  editor: LexicalEditor,
  ...
  editor.getEditorState().read(() => {
    ...
    const nodesToProcess =
      parentNodes.size > 0 ? Array.from(parentNodes) : nodes;
