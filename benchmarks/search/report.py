#!/usr/bin/env python3
"""Generate PNG charts for the Toin search benchmark output."""
...
DEFAULT_INPUT = ROOT / "results" / "search_benchmark_results.json"
...
def main() -> None:
    ...
    paths = [
        metric_averages(args.output_dir, rows, methods),
        llm_distribution(args.output_dir, metrics, methods),
        per_query_llm_average(args.output_dir, rows, methods),
        ...
        baseline_deltas(args.output_dir, rows, methods),
    ]
...
def metric_averages(
    ...
    values_by_method = {
        method: [
            ...
            safe_stat(scores_for(rows, method, "rerank_score_normalized"), median),
        ]
    ...
    ax.set_title("Evaluator Score Summary")
...
def llm_distribution(
    ...
    ax.set_title("LLM Relevance Distribution")
...
def per_query_llm_average(
    out_dir: Path, rows: list[dict[str, Any]], methods: list[str]
) -> Path:
    by_query: dict[str, dict[str, list[float]]] = defaultdict(lambda: defaultdict(list))
    for row in rows:
        score = number(row.get("llm_score"))
        if score is not None:
            by_query[str(row.get("query", ""))][str(row.get("method", ""))].append(
                score
            )
    ...
    values_by_method = {
        method: [
            mean(by_query[query][method]) if by_query[query].get(method) else None
            for query in queries
        ]
        for method in methods
    }

    fig, ax = plt.subplots(figsize=(12, max(5.5, len(queries) * 0.75)))
    grouped_bars(ax, queries, values_by_method, horizontal=True)
    ax.set_title("Average LLM Score by Query")
    ax.set_xlabel("LLM score")
    ax.set_xlim(0, 100)
    ax.grid(axis="x", alpha=0.25)
    return save(fig, out_dir / "per_query_llm_average.png")
...
def latency_summary(
    ...
    grouped_bars(ax, labels, values_by_method)
    ax.set_title("Search Latency Summary")
...
def baseline_deltas(
    ...
    comparisons = [
        ("LLM vs random", "random", "llm_score"),
        ("Rerank vs random", "random", "rerank_score_normalized"),
        ("LLM vs BM25", "bm25", "llm_score"),
        ("Rerank vs BM25", "bm25", "rerank_score_normalized"),
        ("LLM vs vector", "vector", "llm_score"),
        ("Rerank vs vector", "vector", "rerank_score_normalized"),
    ]
    labels = []
    values_by_method: dict[str, list[float | None]] = {method: [] for method in methods}
    for label, baseline, field in comparisons:
        values = [average_delta(rows, method, baseline, field) for method in methods]
    ...
    ax.set_title("Difference from Baselines")
...
def unique_latency_by_method(rows: list[dict[str, Any]]) -> dict[str, list[float]]:
    ...
    for row in rows:
        ...
        key = (str(row.get("query", "")), method, latency)
        if key in seen:
            continue
        seen.add(key)
...
def average_delta(
    ...
    method_scores = scores_for(rows, method, field)
    baseline_scores = scores_for(rows, baseline, field)
    if method == baseline or not method_scores or not baseline_scores:
        return None
    return mean(method_scores) - mean(baseline_scores)
