...
const makeAuthor = (id, name, username) => ({
    id,
    username,
...
const makeCommentsHandler = (comments) => http.get("*/posts/:postId/comments", () => HttpResponse.json(comments));
...
const deleteHandler = http.delete("*/api/comments/:commentId", () => new HttpResponse(null, { status: 204 }));
...
const meta = {
    ...
    tags: ["autodocs"],
    decorators: [
        (Story) => (<div className="w-[600px] p-4 border rounded-lg bg-background text-foreground">
        <Story />
      </div>),
    ],
};
